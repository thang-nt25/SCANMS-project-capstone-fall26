...
export class SubmitKycDto {
  @ApiProperty({ example: '001201012345', description: 'Số Căn cước công dân (CCCD 12 số)' })
  @IsString({ message: 'Số CCCD/CMND không hợp lệ' })
  @IsNotEmpty({ message: 'Số CCCD/CMND không được để trống' })
  idCardNumber: string;

  @ApiProperty({ example: '8012345678', description: 'Mã số thuế thu nhập cá nhân' })
  @IsString({ message: 'Mã số thuế không hợp lệ' })
  @IsNotEmpty({ message: 'Mã số thuế không được để trống' })
  taxCode: string;

  @ApiProperty({ example: 'Vietcombank', description: 'Tên ngân hàng thụ hưởng' })
  ...
  @ApiPropertyOptional({ example: 'KOL chuyên review đồ công nghệ và lifestyle', description: 'Tiểu sử / Giới thiệu' })
  @IsOptional()
  @IsString()
  bio?: string;
}
