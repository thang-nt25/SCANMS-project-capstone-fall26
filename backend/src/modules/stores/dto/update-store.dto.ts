...
export class UpdateStoreDto {
  @ApiPropertyOptional({ example: 'Tech Store Vietnam', description: 'Tên gian hàng' })
  @IsOptional()
  @IsString({ message: 'Tên cửa hàng không hợp lệ' })
  name?: string;

  @ApiPropertyOptional({
    example: 'Chuyên cung cấp thiết bị âm thanh và phụ kiện công nghệ chính hãng',
  ...
  @ApiPropertyOptional({ example: 'https://techstore.vn', description: 'Website chính thức' })
  @IsOptional()
  @IsString()
  websiteUrl?: string;

  @ApiPropertyOptional({ example: 10, description: 'Tỷ lệ % hoa hồng mặc định toàn shop' })
  @IsOptional()
  ...
  @ApiPropertyOptional({ example: 30, description: 'Thời hạn lưu cookie Last-Click (ngày)' })
  @IsOptional()
  ...
  @ApiPropertyOptional({ example: 200000, description: 'Hạn mức rút tối thiểu (VNĐ)' })
  @IsOptional()
