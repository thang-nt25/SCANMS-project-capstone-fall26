...
export class CreateMediaDto {
  @ApiPropertyOptional({ example: null, description: 'ID sản phẩm liên kết (nếu có)' })
  @IsOptional()
  @IsString()
  productId?: string;

  @ApiProperty({ example: 'Kịch bản Video Review TikTok 9:16', description: 'Tiêu đề tài nguyên' })
  ...
  @ApiProperty({
    ...
    description: 'Loại tài nguyên: IMAGE (Banner), VIDEO (Clip review), hoặc COPYWRITE_TEXT (Kịch bản SEO)',
  })
  @IsEnum(AssetType, {
  ...
  @ApiProperty({
    example: '🔥 DEAL SỐC HÔM NAY 🔥\nTai nghe chống ồn ANC Pro X giảm ngay 30% khi mua qua link!',
    description: 'Đường dẫn URL hình ảnh/video hoặc nội dung văn bản kịch bản mẫu',
  })
